...
type Reducer<S, A> = (s: S | void, a: A) => S;
...
export const LanguagesContextProvider = ({ children }: Props) => {
  ...
  const init = (state: Language[]) => {
    if (typeof window !== "undefined") {
      const storage: string[] = JSON.parse(localStorage.getItem("languages"));
      if (storage === null) {
        return state
      }
      ...
      return state.map((l) => {
        return {
          ...l,
          selected: l.name === storage.find((value) => value === l.name),
        };
      });
    }
    return state;
  ...
  return (
    <LanguagesDispatchContext.Provider value={dispatch}>
      <LanguagesStateContext.Provider value={state as State}>
        {children}
